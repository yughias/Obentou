...
static int compare6502(const r2A03_t *a, const r2A03_t *b)
{
    return (a->pc     == b->pc) &&
           (a->a      == b->a)  &&
           (a->s      == b->s)  &&
...
static void set6502(r2A03_t *cpu, cJSON *state)
{
    memset(cpu, 0, sizeof(r2A03_t));
...
int main(void)
{
    for (int i = 0x00; i < 0x100; i++) {
        ...
        snprintf(filename, sizeof(filename),
                 "test/assets/sst_65X02/nes6502/v1/%02x.json", i);
