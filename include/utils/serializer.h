...
#define DESERIALIZE_IS_ARRAY_0_IS_RECURSIVE_0(type, name) \
    if (data + sizeof(obj->name) > end) return NULL; \
    memcpy(&obj->name, data, sizeof(obj->name)); \
    data += sizeof(obj->name);

#define DESERIALIZE_IS_ARRAY_0_IS_RECURSIVE_1(type, name) \
    data = deserialize_##type(&obj->name, data, end); \
    if (data == NULL) return NULL;

#define DESERIALIZE_IS_ARRAY_1_IS_RECURSIVE_0(type, name, arrayLen) \
    if (data + sizeof(obj->name) > end) return NULL; \
    memcpy(obj->name, data, sizeof(obj->name)); \
    data += sizeof(obj->name);

#define DESERIALIZE_IS_ARRAY_1_IS_RECURSIVE_1(type, name, arrayLen) \
    for(int i = 0; i < arrayLen; i++) { \
        data = deserialize_##type(&obj->name[i], data, end); \
        if (data == NULL) return NULL; \
    }
...
static u8* deserialize_##name##_t(name##_t* obj, u8* data, u8* end) { \
    fields \
    return data; \
}
