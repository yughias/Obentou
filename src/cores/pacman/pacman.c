...
{
    p->IN0 = 0xFF;
    if (controls_pressed(CONTROL_PACMAN_UP,    0)) p->IN0 &= ~(u8)(1 << 0);
    if (controls_pressed(CONTROL_PACMAN_LEFT,  0)) p->IN0 &= ~(u8)(1 << 1);
    if (controls_pressed(CONTROL_PACMAN_RIGHT, 0)) p->IN0 &= ~(u8)(1 << 2);
    if (controls_pressed(CONTROL_PACMAN_DOWN,  0)) p->IN0 &= ~(u8)(1 << 3);
    if (controls_pressed(CONTROL_PACMAN_COIN,  0)) p->IN0 &= ~(u8)(1 << 7);

    p->IN1 = 0xFF;
    if (controls_pressed(CONTROL_PACMAN_START1, 0)) p->IN1 &= ~(u8)(1 << 5);
    if (controls_pressed(CONTROL_PACMAN_START2, 0)) p->IN1 &= ~(u8)(1 << 6);
}
...
static void init_aux_board_mspacman(pacman_t* p, const archive_t* archive)
{
    ...
    if (!fu6 || fu6->size < 0x1000 ||
        ...
        p->AUX_INSTALLED = false;
        return;
    }

    p->AUX_INSTALLED = true;
    p->AUX_ROM_LOW   = calloc(1, PACMAN_ROM_SIZE);
    p->AUX_ROM_HIGH  = calloc(1, 0x1800);

    if (!p->AUX_ROM_LOW || !p->AUX_ROM_HIGH) {
        p->AUX_INSTALLED = false;
        free(p->AUX_ROM_LOW);
        free(p->AUX_ROM_HIGH);
        p->AUX_ROM_LOW = p->AUX_ROM_HIGH = NULL;
        return;
    }

    memcpy(p->AUX_ROM_LOW, p->ROM, 0x3000);

    for (u16 i = 0; i < 0x1000; i++) {
        p->AUX_ROM_LOW [0x3000 + decrypt_addr1(i)] = decrypt_byte(fu7->data[i]);
        p->AUX_ROM_HIGH[0x0800 + decrypt_addr1(i)] = decrypt_byte(fu6->data[i]);
    }
    for (u16 i = 0; i < 0x0800; i++)
        p->AUX_ROM_HIGH[decrypt_addr2(i)] = decrypt_byte(fu5->data[i]);
    ...
    for (int k = 0; k < (int)(sizeof(patches)/sizeof(patches[0])); k++)
        for (int b = 0; b < 8; b++)
            p->AUX_ROM_LOW[patches[k][0] + b] = p->AUX_ROM_HIGH[patches[k][1] + b];
}
...
static u8 pacplus_swap_byte(u8 byte, const u8* index)
{
    u8 out = 0;
    for (int i = 0; i < 8; i++) {
...
static u8 decrypt_pacplus(u16 addr, u8 byte)
{
    ...
    return pacplus_swap_byte(byte, tbl) ^ tbl[8];
}
...
typedef struct {
    ...
    bool        pacplus_crypt; /* apply Pac-Plus ROM decryption */
} pacman_romset_t;
...
void* PACMAN_init(const archive_t* rom_archive, const archive_t* bios_archive)
{
    ...
    if (rs->pacplus_crypt)
        for (u16 i = 0; i < PACMAN_ROM_SIZE; i++)
            p->ROM[i] = decrypt_pacplus(i, p->ROM[i]);
    ...
    load_rom(rom_archive, "82s126.1m", p->audioROM,       256);
    load_rom(rom_archive, "82s126.3m", p->audioROM + 256, 256);
    ...
    p->IN0               = 0xFF;
    p->IN1               = 0xFF;
    p->DIP_SWITCH_SETTINGS = 0xF9;  /* 3 lives, medium difficulty */
